...
class DefaultWorkspaceWatcherTest {
    @Test
    fun `emits create event`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
    ...
    @Test
    fun `emits modify event with hash`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
            WorkspaceWatchConfig(
                root = root,
    ...
    @Test
    fun `registers new subdirectories`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
            WorkspaceWatchConfig(
                root = root,
    ...
    @Test
    fun `excludes paths by glob`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
            WorkspaceWatchConfig(
                root = root,
        ...
        try {
            ...
            val event = withTimeoutOrNull(1.seconds) {
                watcher.events.first { it.path == file.toAbsolutePath().normalize() }
            }
    ...
    @Test
    fun `debounces successive events`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
    ...
    @Test
    fun `emits overflow event`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
            WorkspaceWatchConfig(
                root = root,
    ...
    @Test
    fun `handles burst events`() = runBlocking {
        val root = Files.createTempDirectory("dispatch-workspace")
        val watcher = DefaultWorkspaceWatcher(
            WorkspaceWatchConfig(
                root = root,
    ...
    @Test
    fun `follows symlinked directories when enabled`() = runBlocking {
