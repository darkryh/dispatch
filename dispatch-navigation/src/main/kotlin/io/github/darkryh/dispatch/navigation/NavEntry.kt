...
import io.github.darkryh.dispatch.lifecycle.LifecycleRegistry
...
private val stableContentKeyCache: MutableMap<NavKey, String> =
    java.util.Collections.synchronizedMap(
        object : LinkedHashMap<NavKey, String>(64, 0.75f, true) {
            override fun removeEldestEntry(eldest: Map.Entry<NavKey, String>): Boolean = size > STABLE_CONTENT_KEY_CACHE_MAX
        },
    )

@PublishedApi
internal fun stableContentKey(key: NavKey): Any = stableContentKeyCache.getOrPut(key) { encodeNavKeyForSave(key, DefaultRouteJson) }
