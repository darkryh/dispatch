...
class NavDisplayMemoryLeakTest {
    ...
    @Test
    fun `popped entry viewmodel is cleared and its scope cancelled`() = runBlocking {
    ...
    @Test
    fun `back stack does not retain entry state after NavDisplay leaves composition`() = runBlocking {
        val backStack = NavBackStack<LeakRoute>(LeakRoute(1))
    ...
    @Test
    fun `re-pushing same route after pop creates fresh un-cleared state`() = runBlocking {
        val backStack = NavBackStack<LeakRoute>(LeakRoute(1))
