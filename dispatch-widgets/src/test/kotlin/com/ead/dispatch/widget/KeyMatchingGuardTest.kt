 ...
 *
 * Keyboard events must be matched against [com.ead.dispatch.input.Key] values, never by comparing
...
class KeyMatchingGuardTest {
    ...
    fun `no raw key string comparisons in production sources`() {
        ...
        assertTrue(
            offenders.isEmpty(),
            "Raw key-string comparison found (use com.ead.dispatch.input.Key instead): " +
