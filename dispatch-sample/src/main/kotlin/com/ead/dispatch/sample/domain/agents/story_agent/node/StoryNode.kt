...
private fun AIAgentGraphContextBase.setupAndStreamStoryMode(
    ...
    return channelFlow {
        ...
        publishSnapshot()
        try {
            val storyModeContext = withContext(Dispatchers.IO) {
                repository.getStoryModeContext(request.storyId)
            }
            val chatContext = withContext(Dispatchers.IO) {
            ...
            llm.writeSession {
                rewritePrompt { existing ->
                    ...
                    val basePrompt = storyAgentPrompt(
            ...
            while (true) {
                ...
                agentContext.updateStoryTurnMetrics { metrics ->
                    ...
                    effectiveToolCalls.forEach { call ->
                        if (isStoryDecisionToolName(call.tool)) {
                            metrics.decisionToolCalls += 1
                            metrics.selectorShown = true
                        }
                        if (isStoryWriteToolName(call.tool)) {
...
private suspend fun AIAgentGraphContextBase.policyBlockReason(toolName: String?): String? {
    ...
    if (!isStoryToolAllowedForTurn(policy, toolName)) {
        ...
        if (policy.requireSelectorForDestructive && isStoryWriteToolName(toolName)) {
            return "Write tool call blocked: call requestUserChoice first for this destructive turn."
        }
        if (policy.requireSelectorForCreative && isStoryWriteToolName(toolName)) {
            return "Write tool call blocked: call requestUserChoice first to pick a narrative direction."
        }
        return "Tool call blocked by story turn policy."
    }
    return null
}

private fun List<ToolDescriptor>.filterAllowedByPolicy(policy: StoryTurnPolicy): List<ToolDescriptor> =
    filter { descriptor -> isStoryToolAllowedForTurn(policy, descriptor.name) }
...
private fun blockedToolResult(
    ...
    return ReceivedToolResult(
        ...
        tool = call.tool,
        toolArgs = toolArgs,
        toolDescription = "Blocked by story policy guard.",
