...
fun LocationFeatureListScreen(
    ...
    val filteredEntries = remember(state.items, filterQuery) {
        filterEntries(state.items, filterQuery) { item, query ->
            matchesQuery(query, item.name, item.location, item.description)
        }
    }
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(EntityEditorRoute(type = "location-features", storyId = route.storyId, entityId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(EntityEditorRoute(type = "location-features", storyId = route.storyId, entityId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    val titleStyle = rgb("#B5E48C") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "Location Features", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
            ListFilterBar(state = filterField, placeholder = "Filter by feature, location, or tags...")
        }
        item { Spacer(Modifier.height(1)) }
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading location features...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
