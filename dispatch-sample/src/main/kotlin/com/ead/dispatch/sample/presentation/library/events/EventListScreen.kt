...
fun EventListScreen(
    ...
    val viewModel = viewModel<EventListViewModel>()
    ...
    fun openSelected() {
        ...
        when (entry) {
            ...
            is ListEntry.Item -> {
                navigator.navigate(EventEditorRoute(storyId = route.storyId, eventId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    val titleStyle = rgb("#F58D42") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "Events", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
            FilterBarCard(
                state = filterField,
                placeholder = "Filter by event, date, or impact...",
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading events...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
