...
fun RelationshipListScreen(
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(RelationshipEditorRoute(storyId = route.storyId, relationshipId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(RelationshipEditorRoute(storyId = route.storyId, relationshipId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            FilterBarCard(
                state = filterField,
                placeholder = "Filter by characters, type, or status...",
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading relationships...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
