...
@Dispatchable
fun OrganizationEditorScreen(backStack: NavBackStack<NavKey>, route: OrganizationEditorRoute) {
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<OrganizationEditorViewModel>()
    ...
    DisposableEffect(Unit) {
        val dispose = keyboardInterceptor.register { event ->
            when (event.key) {
                ...
                "S", "s" -> if (event.ctrl) {
                    viewModel.onEvent(OrganizationEditorEvent.OnSave)
                    true
                } else false
                "D", "d" -> if (event.ctrl) {
                    viewModel.onEvent(OrganizationEditorEvent.OnRequestDelete)
                    true
                } else false
                "X", "x" -> if (event.ctrl) {
                    viewModel.onEvent(OrganizationEditorEvent.OnRequestDelete)
                ...
                "Y", "y" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(OrganizationEditorEvent.OnConfirmDelete)
                    true
                } else false
                "N", "n" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(OrganizationEditorEvent.OnCancelDelete)
                    true
                } else false
                "Tab" -> if (event.shift) {
                    viewModel.onEvent(OrganizationEditorEvent.OnToggleMode)
                    true
                } else false
                "G", "g" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(OrganizationEditorEvent.OnGenerateAiDraft)
                    true
                } else false
                "A", "a" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(OrganizationEditorEvent.OnApplyAiDraft)
                    true
                } else false
                "R", "r" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(OrganizationEditorEvent.OnRegenerateAiDraft)
                    true
                } else false
                "Q", "q" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(OrganizationEditorEvent.OnToggleAiMode)
                    true
                } else false
                else -> false
            }
    ...
    val manualFields = OrganizationEditorFields.manual
    val automaticFields = EditorAiFields.fields

    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        item {
            EditorHeader(
                title = if (uiState.mode == EditorMode.MANUAL) "Organization (Manual)" else "Organization (AI Prompt)",
                styles = styles,
            )
        }
        ...
        if (uiState.isLoading) {
            item { EditorFooter(text = "Loading organization...", styles = styles) }
            return@LazyColumn
        }
        ...
        if (uiState.confirmDelete) {
            item {
                EditorFooter(text = "Delete this organization? Press Y to confirm or N to cancel.", styles = styles)
            }
            item { Spacer(Modifier.height(1)) }
        }
        ...
        if (uiState.mode == EditorMode.MANUAL) {
            ...
            item {
                ManualEditorForm(
                    ...
                    onValueChange = { key, text ->
                        viewModel.onEvent(OrganizationEditorEvent.OnFieldChanged(key, text))
                    },
                )
            }
            item { Spacer(Modifier.height(1)) }
            ...
            if (draft != null) {
                ...
                item { Spacer(Modifier.height(1)) }
                item { OrganizationAIDraftPreview(draft = draft, styles = styles) }
            }
            item { Spacer(Modifier.height(1)) }
            item {
                EditorFooter(
