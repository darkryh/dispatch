...
@Dispatchable
fun ChapterListScreen(route: ChapterListRoute) {
    val navigator = LocalNavigator.current
    val theme = LocalTheme.current
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val terminalHeight = LocalTerminalHeight.current
    val viewModel = viewModel<ChapterListViewModel>()
    ...
    val titleStyle = rgb("#A6C8FF") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text("Chapters", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text("Loading chapters...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
