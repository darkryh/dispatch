...
fun ArtifactListScreen(
    ...
    route: ArtifactListRoute
) {
    val theme = LocalTheme.current
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<ArtifactListViewModel>()
    ...
    val filteredEntries = remember(state.items, filterQuery) {
        filterEntries(state.items, filterQuery) { item, query ->
            matchesQuery(query, item.name, item.owner, item.location, item.description)
        }
    }
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(EntityEditorRoute(type = "artifacts", storyId = route.storyId, entityId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(EntityEditorRoute(type = "artifacts", storyId = route.storyId, entityId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    val titleStyle = rgb("#FFD166") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "Artifacts", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading artifacts...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
