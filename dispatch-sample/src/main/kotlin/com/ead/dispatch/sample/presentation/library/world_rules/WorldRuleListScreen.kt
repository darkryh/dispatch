...
@Dispatchable
fun WorldRuleListScreen(
    backStack: NavBackStack<NavKey>,
    route: WorldRuleListRoute
) {
    val theme = LocalTheme.current
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<WorldRuleListViewModel>()
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(EntityEditorRoute(type = "world-rules", storyId = route.storyId, entityId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(EntityEditorRoute(type = "world-rules", storyId = route.storyId, entityId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    val titleStyle = rgb("#FF8E8E") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "World Rules", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
            ListFilterBar(state = filterField, placeholder = "Filter by rule, domain, or constraint...")
        }
        item { Spacer(Modifier.height(1)) }
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading world rules...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
                ...
                SelectableWindowedList(
                    ...
                    when (entry) {
                        ...
                        is ListEntry.Item -> {
                            ...
                            Text(
                                text = "constraint: ${item.description}",
                                style = if (isSelected) selectedMetaStyle else metaStyle,
                                maxLines = 1,
                                overflow = TextOverflow.Ellipsis,
                            )
