...
fun LocationListScreen(
    ...
    val filteredEntries = remember(state.items, filterQuery) {
        filterEntries(state.items, filterQuery) { item, query ->
            matchesQuery(query, item.name, item.description, item.tags)
        }
    }
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(LocationEditorRoute(storyId = route.storyId, locationId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(LocationEditorRoute(storyId = route.storyId, locationId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item { Spacer(Modifier.height(1)) }
        item {
            ListFilterBar(state = filterField, placeholder = "Filter by name, tags, or description...")
        }
        item { Spacer(Modifier.height(1)) }
