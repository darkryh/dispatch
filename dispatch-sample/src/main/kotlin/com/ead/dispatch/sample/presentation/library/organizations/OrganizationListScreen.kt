...
fun OrganizationListScreen(
    ...
    route: OrganizationListRoute
) {
    val theme = LocalTheme.current
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<OrganizationListViewModel>()
    ...
    fun openSelected() {
        ...
        when (entry) {
            is ListEntry.Create -> {
                backStack.navigate(EntityEditorRoute(type = "organizations", storyId = route.storyId, entityId = null))
            }
            is ListEntry.Item -> {
                backStack.navigate(EntityEditorRoute(type = "organizations", storyId = route.storyId, entityId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "Organizations", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
            ListFilterBar(state = filterField, placeholder = "Filter by name, type, or influence...")
        }
        item { Spacer(Modifier.height(1)) }
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading organizations...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
