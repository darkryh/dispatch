...
fun TimelineListScreen(
    route: TimelineListRoute
) {
    val navigator = LocalNavigator.current
    val theme = LocalTheme.current
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<TimelineListViewModel>()
    ...
    val filteredEntries = remember(state.items, filterQuery) {
        filterEntries(state.items, filterQuery) { item, query ->
            matchesQuery(query, item.title, item.description, item.order.toString())
        }
    }
    ...
    fun openSelected() {
        ...
        when (entry) {
            ...
            is ListEntry.Item -> {
                navigator.navigate(TimelineEditorRoute(storyId = route.storyId, timelineEntryId = entry.data.id))
            }
            null -> Unit
        }
    }
    ...
    val titleStyle = rgb("#9BF6FF") + TextStyle(bold = true)
    ...
    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(text = "Timeline", style = theme.primary + TextStyle(bold = true))
            }
        }
        item { Spacer(Modifier.height(1)) }
        item {
            FilterBarCard(
                state = filterField,
                placeholder = "Filter by entry, date, or era...",
        ...
        if (state.isLoading) {
            item {
                Row(modifier = Modifier.fillMaxWidth()) {
                    Spacer(Modifier.width(2))
                    Text(text = "Loading timeline...", style = theme.muted)
                }
            }
            return@LazyColumn
        }
