...
@Dispatchable
fun CultureEditorScreen(backStack: NavBackStack<NavKey>, route: CultureEditorRoute) {
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<CultureEditorViewModel>()
    ...
    DisposableEffect(Unit) {
        val dispose = keyboardInterceptor.register { event ->
            when (event.key) {
                ...
                "S", "s" -> if (event.ctrl) {
                    viewModel.onEvent(CultureEditorEvent.OnSave)
                    true
                } else false
                "D", "d" -> if (event.ctrl) {
                    viewModel.onEvent(CultureEditorEvent.OnRequestDelete)
                    true
                } else false
                "X", "x" -> if (event.ctrl) {
                    viewModel.onEvent(CultureEditorEvent.OnRequestDelete)
                ...
                "Y", "y" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(CultureEditorEvent.OnConfirmDelete)
                    true
                } else false
                "N", "n" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(CultureEditorEvent.OnCancelDelete)
                    true
                } else false
                "Tab" -> if (event.shift) {
                    viewModel.onEvent(CultureEditorEvent.OnToggleMode)
                    true
                } else false
                "G", "g" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(CultureEditorEvent.OnGenerateAiDraft)
                    true
                } else false
                "A", "a" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(CultureEditorEvent.OnApplyAiDraft)
                    true
                } else false
                "R", "r" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(CultureEditorEvent.OnRegenerateAiDraft)
                    true
                } else false
                "Q", "q" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(CultureEditorEvent.OnToggleAiMode)
                    true
                } else false
                else -> false
            }
    ...
    val manualFields = CultureEditorFields.manual
    val automaticFields = EditorAiFields.fields

    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        item {
            EditorHeader(
                title = if (uiState.mode == EditorMode.MANUAL) "Culture (Manual)" else "Culture (AI Prompt)",
                styles = styles,
            )
        }
        ...
        if (uiState.isLoading) {
            item { EditorFooter(text = "Loading culture...", styles = styles) }
            return@LazyColumn
        }
        ...
        if (uiState.confirmDelete) {
            item {
                EditorFooter(text = "Delete this culture? Press Y to confirm or N to cancel.", styles = styles)
            }
            item { Spacer(Modifier.height(1)) }
        }
        ...
        if (uiState.mode == EditorMode.MANUAL) {
            ...
            item {
                ManualEditorForm(
                    ...
                    onValueChange = { key, text ->
                        viewModel.onEvent(CultureEditorEvent.OnFieldChanged(key, text))
                    },
                )
            }
            item { Spacer(Modifier.height(1)) }
            ...
            if (draft != null) {
                ...
                item { Spacer(Modifier.height(1)) }
                item { CultureAIDraftPreview(draft = draft, styles = styles) }
            }
            item { Spacer(Modifier.height(1)) }
            item {
                EditorFooter(
