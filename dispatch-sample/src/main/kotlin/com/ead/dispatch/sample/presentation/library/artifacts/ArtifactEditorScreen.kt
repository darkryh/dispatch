...
@Dispatchable
fun ArtifactEditorScreen(backStack: NavBackStack<NavKey>, route: ArtifactEditorRoute) {
    val keyboardInterceptor = LocalKeyboardInterceptor.current
    val viewModel = viewModel<ArtifactEditorViewModel>()
    ...
    DisposableEffect(Unit) {
        val dispose = keyboardInterceptor.register { event ->
            when (event.key) {
                ...
                "S", "s" -> if (event.ctrl) {
                    viewModel.onEvent(ArtifactEditorEvent.OnSave)
                    true
                } else false
                "D", "d" -> if (event.ctrl) {
                    viewModel.onEvent(ArtifactEditorEvent.OnRequestDelete)
                    true
                } else false
                "X", "x" -> if (event.ctrl) {
                    viewModel.onEvent(ArtifactEditorEvent.OnRequestDelete)
                ...
                "Y", "y" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(ArtifactEditorEvent.OnConfirmDelete)
                    true
                } else false
                "N", "n" -> if (uiState.confirmDelete) {
                    viewModel.onEvent(ArtifactEditorEvent.OnCancelDelete)
                    true
                } else false
                "Tab" -> if (event.shift) {
                    viewModel.onEvent(ArtifactEditorEvent.OnToggleMode)
                    true
                } else false
                "G", "g" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(ArtifactEditorEvent.OnGenerateAiDraft)
                    true
                } else false
                "A", "a" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(ArtifactEditorEvent.OnApplyAiDraft)
                    true
                } else false
                "R", "r" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(ArtifactEditorEvent.OnRegenerateAiDraft)
                    true
                } else false
                "Q", "q" -> if (event.ctrl && uiState.mode == EditorMode.AUTOMATIC) {
                    viewModel.onEvent(ArtifactEditorEvent.OnToggleAiMode)
                    true
                } else false
                else -> false
            }
    ...
    val manualFields = ArtifactEditorFields.manual
    val automaticFields = EditorAiFields.fields

    LazyColumn(modifier = Modifier.fillMaxWidth()) {
        item {
            EditorHeader(
                title = if (uiState.mode == EditorMode.MANUAL) "Artifact (Manual)" else "Artifact (AI Prompt)",
                styles = styles,
            )
        }
        ...
        if (uiState.isLoading) {
            item { EditorFooter(text = "Loading artifact...", styles = styles) }
            return@LazyColumn
        }
        ...
        if (uiState.confirmDelete) {
            item {
                EditorFooter(text = "Delete this artifact? Press Y to confirm or N to cancel.", styles = styles)
            }
            item { Spacer(Modifier.height(1)) }
        }
        ...
        if (uiState.mode == EditorMode.MANUAL) {
            ...
            item {
                ManualEditorForm(
                    ...
                    onValueChange = { key, text ->
                        viewModel.onEvent(ArtifactEditorEvent.OnFieldChanged(key, text))
                    },
                )
            }
            item { Spacer(Modifier.height(1)) }
            ...
            if (draft != null) {
                ...
                item { Spacer(Modifier.height(1)) }
                item { ArtifactAIDraftPreview(draft = draft, styles = styles) }
            }
            item { Spacer(Modifier.height(1)) }
            item {
                EditorFooter(
