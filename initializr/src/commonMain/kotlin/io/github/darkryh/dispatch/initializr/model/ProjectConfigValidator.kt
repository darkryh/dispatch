 ...
 * decide whether to enable the "Generate" button and which messages to show. Rules are intentionally
 * strict enough that the generated Gradle project is guaranteed to have a legal package, artifact id,
 * and coordinates — so the download always compiles (once the Dispatch artifacts are published).
 */
    ...
    fun validate(config: ProjectConfig): List<FieldError> =
        buildList {
            ...
            if (config.artifactId.isBlank()) {
                ...
                add(FieldError(ConfigField.ARTIFACT_ID, "Use lowercase kebab-case, e.g. my-dispatch-app."))
            }
            ...
            when {
                ...
                config.packageName.split('.').any { it in KOTLIN_KEYWORDS } ->
                    add(FieldError(ConfigField.PACKAGE_NAME, "A package segment is a reserved keyword."))
            }
