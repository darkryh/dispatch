...
class TerminalRendererConcurrencyTest {
    ...
    @Test
    fun `concurrent cursor operations do not interleave with active area updates`() = runBlocking {
    ...
    @Test
    fun `clearScreen during active area update does not corrupt state`() = runBlocking {
        val (renderer, _) = createRenderer()
        val iterations = 50
    ...
    @Test
    fun `rapid updateActiveArea calls are properly serialized`() = runBlocking {
        val (renderer, _) = createRenderer()
        val iterations = 200
    ...
    @Test
    fun `cursor visibility is consistent after concurrent operations`() = runBlocking {
        val (renderer, _) = createRenderer()
        val iterations = 100
    ...
    @Test
    fun `appendScrollingContent and updateActiveArea do not race`() = runBlocking {
        val (renderer, _) = createRenderer()
        val iterations = 100
    ...
    @Test
    fun `mixed operations under heavy load do not deadlock`() = runBlocking {
        val (renderer, _) = createRenderer()
        val iterations = 50
